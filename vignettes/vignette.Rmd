...
metrics.to.plot <- crm$detailed_metrics$metric %>%
  unique()
crm$plotDetailedMetrics(metrics = metrics.to.plot, plot_geom = "violin")
```
...
```{r}
crm$plotDetailedMetrics(metrics = c("depth"), plot_geom = "point")
crm$plotDepth()
```

```{r}
crm$plotDepth(per_sample = TRUE)
```
...
depth_cutoff_vec <- c(1e3,1e3,1e3,1e3,3e3,3.5e3) %>% 
  setNames(crm$detailed_metrics$sample %>% unique())
...
```{r}
crm$plotDepth(cutoff = depth_cutoff_vec, per_sample = TRUE)
```
...
```{r}
crm$plotFilteredCells(type = "bar", doublet_method = "scrublet")
```
